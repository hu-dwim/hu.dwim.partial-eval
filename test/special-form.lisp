...
(def test xxx ()
  (partial-eval '(let ((text-1 "he"))
                  (block nil
                    (let ((char-1 nil)
                          (loop-across-vector-2020 text-1)
                          (loop-across-index-2021 0)
                          (loop-across-limit-2022 0))
                      (let ((char-2 nil)
                            (loop-across-vector-2023 text-2)
                            (loop-across-index-2024 0)
                            (loop-across-limit-2025 0))
                        (tagbody
                           (setq loop-across-limit-2022
                                 (length loop-across-vector-2020))
                           (setq loop-across-limit-2025
                                 (length loop-across-vector-2023))
                         :begin
                           (if (>= loop-across-index-2021 loop-across-limit-2022)
                               (progn (go :end))
                               nil)
                           (setq char-1
                                 (aref loop-across-vector-2020
                                       loop-across-index-2021))
                           (setq loop-across-index-2021 (1+ loop-across-index-2021))
                           (if (>= loop-across-index-2024 loop-across-limit-2025)
                               (progn (go :end))
                               nil)
                           (setq char-2
                                 (aref loop-across-vector-2023
                                       loop-across-index-2024))
                           (setq loop-across-index-2024 (1+ loop-across-index-2024))
